...
contract Promissory {
    ...
    modifier checkPromissoryOwner(){    
        if(msg.sender != promissoryOwner) revert("Caller is not the owner");
        _;
    }

    function approveProperty(uint256 _propertyId) external checkPromissoryOwner() {
        require(properties[_propertyId].status == PropertyStatus.ADDED, "Property don't exist!");
    ...
    function investInProperty(uint256 _propertyId, uint256 _investmentAmount) public {
        require(properties[_propertyId].status == PropertyStatus.APPROVED, "Property isn't approved");
        require(_investmentAmount <= lockedTokens[_propertyId], "Amount exceeds available Property Tokens");
    ...
    function claimInvestment(uint256 _propertyId, uint256 _numberOfTokensToClaim) external {
        require(msg.sender == properties[_propertyId].owner, "Not the onwer!");
    ...
    function returnInvestment(uint256 _propertyId, uint256 _investmentId) external {
        require(msg.sender == properties[_propertyId].owner, "Not the owner!");
    ...
    function claimReturn(uint256 _propertyId, uint256 _investmentId) external {
        require(msg.sender == (investments[_propertyId][_investmentId]).investor, "You havn't invested in this property!");
    ...
    function claimPropertyTokens(uint256 _propertyId, uint256 _claimTokens) external {
        require(msg.sender == properties[_propertyId].owner, "Not the owner!");
        require(_claimTokens <= lockedTokens[_propertyId], "Claiming more tokens than locked!");
    ...
    function updateInterestRate(uint _propertyId, uint256 _interestRate) external {
        require(properties[_propertyId].status == PropertyStatus.ADDED, "Property isn't APPROVED!");
